...
// 设计原则：机位全部位于站房后方走廊深处（z>=2.5，设备密集区在 z=0~1），且抬高到管道上方（y>=2.5），
// 通过“走廊高处远眺”展示设备，绝不进入管道包围盒，从源头避免穿模。
const TOUR_KF = [
  { pos: [4.50, 2.60, 3.50], look: [7.00, 1.60, -2.50] },   // 0 开场：走廊深处高处，俯瞰设备区
  { pos: [5.50, 2.60, 3.00], look: [7.50, 1.60, -3.00] },   // 1 缓移：看整体 1:1 还原
  { pos: [6.50, 2.50, 2.50], look: [8.00, 1.60, -3.50] },   // 2 推进：继续沿走廊远眺
  { pos: [7.00, 1.90, 2.20], look: [8.20, 1.60, -4.00] },   // 3 第一人称：仍站在走廊后方，望向通道深处
  { pos: [8.00, 2.00, 1.50], look: [8.50, 1.60, -4.50] },   // 4 沿设备列滑行：保持在走廊，看设备侧面
  { pos: [7.00, 2.20, 2.20], look: [6.14, 1.29, 0.13] },    // 5 看循环泵：走廊远处正视，不接近管道
  { pos: [5.50, 2.70, 3.00], look: [7.00, 1.50, -2.00] },   // 6 抬升：多设备彩色看板总览
  { pos: [5.50, 2.30, -1.00], look: [6.30, 1.93, -4.10] },  // 7 换热器前：保持在走廊末端正视
  { pos: [5.50, 2.30, -1.00], look: [6.30, 1.93, -4.10] },  // 8 停留，逐点拾取坐标
  { pos: [4.50, 2.70, 3.00], look: [6.50, 1.50, -2.50] },   // 9 后拉：未来功能卡（仍站内走廊）
  { pos: [4.00, 2.80, 3.50], look: [6.50, 1.40, -2.50] },   // 10 收尾：项目卡（仍站内走廊）
];
const TOUR_START = { pos: [4.00, 2.60, 4.00], look: [6.80, 1.60, -2.50] }; // 起点：走廊深处，远离设备
