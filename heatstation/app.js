...
// 设计原则（多样性 + 安全双达标，analyze_path.mjs 验证 VERDICT PASS）：
//  - 安全：所有机位经“真实扫描模型 8 向水平射线 + 向下射线”验证，位于房间内、与表面净距≥0.46m、
//    注视方向命中真实几何体（不空白）；高位机位靠“脚下有地面”合法化（不再误判 void）。
//  - 多样性：机位 y 在 0.95~1.65 间起伏；景别距离 1.3~5.7m（含推近特写与拉远全景）；
//    含 2 段“环视停顿”（pos 不变、仅 look 横扫）；路径非共线、弯曲度≈440°。
const TOUR_KF = [
  { pos: [2.30, 1.00, -0.10], look: [6.138, 1.289, 0.132] },  // 0 二级泵#1（中景·平视）
  { pos: [3.50, 1.00, -0.20], look: [6.153, 1.330, 0.132] },  // 1 二级泵#1 推近特写（退 x3.5 远离红绿主管道）
  { pos: [3.50, 0.95, -0.55], look: [6.153, 1.330, 1.339] },  // 2 二级泵#2（低姿·沿走廊 z 偏移成弧线）
  { pos: [3.50, 0.95, -0.55], look: [7.236, 1.755, -3.036] }, // 3 环视停顿①：机位不动，look 横扫至循环泵#1
  { pos: [3.45, 1.10, -0.95], look: [8.282, 1.956, -2.901] }, // 4 循环泵#2（退 x3.45 远离管道·机位抬升 y1.1）
  { pos: [3.45, 1.00, -1.40], look: [7.880, 1.341, -4.914] }, // 5 补水泵#1（退 x3.45 留净距+避开白色立柱视线）
  { pos: [3.45, 1.00, -1.40], look: [7.917, 1.353, -5.273] }, // 6 环视停顿②：机位不动，look 横扫至补水泵#2
  { pos: [3.80, 1.65, -2.00], look: [3.828, 2.385, -7.533] }, // 7 液位传感器（高位俯视 y1.65，脚下有地面合法化）
  { pos: [4.70, 1.20, -2.00], look: [4.555, 2.910, 2.616] },  // 8 热表（z=-2.0 通道·仰视）
  { pos: [5.40, 1.55, -1.95], look: [6.300, 1.600, -3.000] }, // 9 拉远全景复现（高位 y1.55）
  { pos: [5.55, 1.00, -2.05], look: [6.200, 1.600, -3.000] }, // 10 收尾（项目卡·平视）
];
const TOUR_START = { pos: [1.85, 1.55, 0.60], look: [6.40, 1.40, -2.00] }; // 起点：高位宽景 establisher（脚下有地面，合法）
...
function tourUpdate() {
  ...
  // 机位已用 analyze_path.mjs 对真实扫描模型 8 向射线预验证（净距≥0.46m、0 穿模、0 空白），
  // 导览期间【不再每帧调用 clampTourPosition】——3DGS 射线距离在狭窄走廊有噪声，斥力场会把
  // 相机子秒级来回推，产生“抽动”。仅保留极端回退：若插值点意外进入几何体<0.2m 才推出一次。
  if (viewer && viewer.splatMesh) {
    let worst = Infinity;
    for (const d of _tourDirs) { const dist = castSplatDist(_pp, d); if (dist < worst) worst = dist; }
    if (worst < 0.2) { // 极端保险，正常情况下不触发
      for (let iter = 0; iter < 2; iter++) {
        let dx = 0, dz = 0, active = false;
        for (const d of _tourDirs) { const dist = castSplatDist(_pp, d); if (dist < 0.2) { const pen = 0.2 - dist; dx -= d.x * pen; dz -= d.z * pen; active = true; } }
        if (!active) break; _pp.x += dx; _pp.z += dz;
      }
    }
  }
