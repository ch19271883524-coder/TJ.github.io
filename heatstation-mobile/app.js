...
function roamMove() {
  ...
  if (joy.id !== null) {                       // 移动端摇杆：y(上推为负)=前进，x(右)=右移
    if (joy.y) tryMove(dir.clone().multiplyScalar(-joy.y * sp));
    if (joy.x) tryMove(right.clone().multiplyScalar(joy.x * sp));
  }
  applyLook();
}
...
// ---------- 移动端触控（仅在触屏设备生效；控件由 CSS @media(pointer:coarse) 显示） ----------
const joyEl = document.getElementById('joy');
const joyKnob = document.getElementById('joyKnob');
const btnUp = document.getElementById('btnUp');
const btnDown = document.getElementById('btnDown');
const joy = { id:null, x:0, y:0, cx:0, cy:0, R:34 };
const look = { id:null, x:0, y:0 };
function isTouchUI(el) { return el && (el.closest('#joy') || el.closest('#btnUp') || el.closest('#btnDown')); }
viewerEl.addEventListener('touchstart', e => {
  for (const t of e.changedTouches) {
    if (t.target && t.target.closest('#joy')) {
      const r = joyEl.getBoundingClientRect();
      joy.id = t.identifier; joy.cx = r.left + r.width / 2; joy.cy = r.top + r.height / 2; joy.x = 0; joy.y = 0;
      e.preventDefault();
    } else if (!isTouchUI(t.target)) {
...
viewerEl.addEventListener('touchmove', e => {
  for (const t of e.changedTouches) {
    if (t.identifier === joy.id) {
      ...
      yaw -= dx * 0.005; pitch = THREE.MathUtils.clamp(pitch - dy * 0.005, -1.5, 1.5);
      applyLook();
      e.preventDefault();
    }
  }
...
function endTouch(e) {
  for (const t of e.changedTouches) {
    if (t.identifier === joy.id) { joy.id = null; joy.x = 0; joy.y = 0; joyKnob.style.transform = 'translate(0,0)'; }
    else if (t.identifier === look.id) { look.id = null; }
  }
}
viewerEl.addEventListener('touchend', endTouch);
viewerEl.addEventListener('touchcancel', endTouch);
// ▲▼ 升降：按住持续升降，同时兼容桌面鼠标
...
holdBtn(btnUp, 'e'); holdBtn(btnDown, 'q');
